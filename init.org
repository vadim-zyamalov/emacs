...
При помощи данного кода загружается и активируется пакетный менеджер =elpaca=.
...
В Windows ограничение на число создаваемых подпроцессов достаточно жесткое, поэтому установим ограничение в 10 подпроцессов.
...
[[https://github.com/progfolio/elpaca/blob/master/extensions/elpaca-use-package.el][elpaca-use-package]] добавляет ключевое слово =:elpaca= в =use-package=, подобное стандартному =:ensure=.

[[https://github.com/abo-abo/hydra][Hydra]] позволяет снизить число нажатий клавиш при цепочке последовательных нажатий: достаточно ввести начальную комбинацию, затем ее можно опустить. [[https://gitlab.com/to1ne/use-package-hydra][use-package-hydra]] добавляет ключевое слово =:hydra=.
